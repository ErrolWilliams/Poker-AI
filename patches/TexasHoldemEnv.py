...
class TexasHoldemEnv(roomai.common.AbstractEnv):
    ...
    def init(self, params = dict()):
        ...
        if "big_blind_bet" in params:
            ...
            self.__params__["big_blind_bet"] = 20
...
#####################################Utils Function ##############################
    ...
    def available_actions(cls, public_state, person_state):
        ...
        if pu.chips[turn] > pu.max_bet_sofar - pu.bets[turn]:
            num = int((pu.chips[turn] - (pu.max_bet_sofar - pu.bets[turn])) / 10)#pu.raise_account)
            for i in range(1, num + 1):
                price = pu.max_bet_sofar - pu.bets[turn] + 10 * i
